...
namespace Form_UI
{
    public partial class PabloForm : Form
    {
        User _user;
        UserDetail _userDetail;
        List<OrderDetail> _orderDetails;
        List<Coffee> _pabloCoffees;
        ...
        public PabloForm(ICafeService cafeManager, ICoffeeService coffeeManager, IOrderService orderManager, IOrderDetailService orderDetailManager, IUserService userManager, IUserDetailService userDetailManager, User user, UserDetail userDetail, List<OrderDetail> orderDetails)
        {
            InitializeComponent();
        ...
        private void PabloForm_Load(object sender, EventArgs e)
        {
            if (_userDetail != null)
            {
            ...
            var result = _coffeeManager.GetAllByCafeId(4);
            if (result.Success)
            {
                _pabloCoffees = result.Data;
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[0];
            AddToCard(coffee, amount1);
        }
        private void button2_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[1];
            AddToCard(coffee, amount2);
        }
        private void button3_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[2];
            AddToCard(coffee, amount3);
        }
        private void button4_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[3];
            AddToCard(coffee, amount4);
        }
        private void button5_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[4];
            AddToCard(coffee, amount5);
        }
        private void button6_Click(object sender, EventArgs e)
        {
            var coffee = _pabloCoffees[5];
