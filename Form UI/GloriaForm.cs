...
namespace Form_UI
{
    public partial class GloriaForm : Form
    {
        User _user;
        UserDetail _userDetail;
        List<OrderDetail> _orderDetails;
        List<Coffee> _gloriaCoffees;
        ...
        public GloriaForm(ICafeService cafeManager, ICoffeeService coffeeManager, IOrderService orderManager, IOrderDetailService orderDetailManager, IUserService userManager, IUserDetailService userDetailManager, User user, UserDetail userDetail, List<OrderDetail> orderDetails)
        {
            InitializeComponent();
        ...
        private void GloriaForm_Load(object sender, EventArgs e)
        {
            if (_userDetail != null)
            {
            ...
            var result = _coffeeManager.GetAllByCafeId(5);
            if (result.Success)
            {
                _gloriaCoffees = result.Data;
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[0];
            AddToCard(coffe, amount1);
        }
        private void button2_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[1];
            AddToCard(coffe, amount2);
        }
        private void button3_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[2];
            AddToCard(coffe, amount3);
        }
        private void button4_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[3];
            AddToCard(coffe, amount4);
        }
        private void button5_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[4];
            AddToCard(coffe, amount5);
        }
        private void button6_Click(object sender, EventArgs e)
        {
            var coffe = _gloriaCoffees[5];
            AddToCard(coffe, amount6);
        }
